...
console.log('SETUP_COMPLETE', gateway.config.get('SETUP_COMPLETE'));

if (gateway.config.get('SETUP_COMPLETE')) {
  function auth() {
    return passport.authenticate('adminBasic', {session: false });
  }

  app.get('/v1/setup', auth(), publicCtrl.setupStatus);
  app.post('/v1/setup', auth(), publicCtrl.setupWizard);
} else {
  app.get('/v1/setup', publicCtrl.setupStatus);
  app.post('/v1/setup', publicCtrl.setupWizard);
}
