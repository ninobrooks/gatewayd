    ...
    if(err){
      ...
      self._setHotWallet(function(err, config){
        if(err){
          ...
          self._fundHotWallet(configProperties, 'sp1RTbeq9djvXFyGfmS2v3XMKcgVa', function(err, config){
            if(err){
              callback(err, null);
            } else {
...
Wizard.prototype._fundHotWallet = function (configProperties, secret, callback){
  var self = this;
  api.fundHotWallet(50, 'XRP', secret, function(err, lines){
    if(err){
      callback(err, null);
    } else {
...
Wizard.prototype._setTrustLine = function(config, callback){
  ...
  for (var currency in config.currencies) {
    api.setTrustLine(currency, 1000, function(err, trust){;
      if(err){
        callback(err, null);
      } else {
        self.setupConfig.trust = trust;
        callback(null, self.setupConfig);
      }
    });
  }
