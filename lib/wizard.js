...
Wizard.prototype._verifyPostgresConnection = function(callback) {
  ...
  db
    ...
    .complete(function(err){
      ...
      if(err){
        self.errors.push({ field: 'database', message: 'database is not connected' });
      } else {
        callback(null, true);
      }
      if(self.errors.length > 0){
...
Wizard.prototype._verifyRippleRestConnection = function(rippleRestUrl, callback){
  ...
  self.client.api = rippleRestUrl;

  self.client.ping(function(err, body){
    if(err){
      self.errors.push({ field: 'ripple_rest', message: 'ripple rest is not running' });
...
Wizard.prototype._checkAccountBalance = function(coldWalletAddress, callback){
  var self = this;
  self.client.account = coldWalletAddress;

  self.client.getAccountBalance(function(err, balance){
    if(err){

      self.errors.push({ field: 'ripple_address', message: 'account balance not available'});

    } else {
      var validBalance = Number(balance.balances[0].value) >= 100;
      if (!validBalance){
        self.errors.push({ field: 'ripple_address', message: 'account balance must be at least 100 XRP'});
      } else {
